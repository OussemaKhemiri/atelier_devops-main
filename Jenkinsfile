    ...
    stages {
        ...
        stage('Checkout & Info') {
            steps {
                checkout scm 
                script {
                    // Capture commit message using a different approach
        ...
        stage('Generate Executive Report') {
            steps {
                script {
                    // First get all values in local variables
                    def reportDate = sh(returnStdout: true, script: 'date "+%Y-%m-%d %H:%M"').trim()
                    def buildStatus = currentBuild.currentResult ?: 'SUCCESS'
                    
                    // Determine colors
                    def statusColor = buildStatus == 'FAILURE' ? '#c0392b' : '#27ae60'
                    def badgeClass = buildStatus == 'FAILURE' ? 'badge-danger' : 'badge-success'
                    
                    // Set environment variables
                    env.REPORT_DATE = reportDate
                    env.BUILD_STATUS = buildStatus
                    env.STATUS_COLOR = statusColor
                    env.BADGE_CLASS = badgeClass
                }
